    ...
    for model, task_rows in by.items():
        ...
        out[model] = {
            **summarize_execution_schema(rs),
            ...
            'max_host_memory_used_bytes': mx('max_host_memory_used_bytes'),
            'max_host_memory_pct': mx('max_host_memory_pct'),
            'max_gpu_memory_used_bytes': mx('max_gpu_memory_used_bytes'),
        ...
        def summarize_subset(sub_rows):
            ...
            return {
                **summarize_execution_schema(sub_rows),
                ...
                'max_host_memory_used_bytes': sub_mx('max_host_memory_used_bytes'),
                'max_host_memory_pct': sub_mx('max_host_memory_pct'),
                'max_gpu_memory_used_bytes': sub_mx('max_gpu_memory_used_bytes'),
...
def generate_model_detail_pages(models, direct_summary, standardized_summary, oc_summary, direct_rank, direct_total, standardized_rank, standardized_total, oc_rank, oc_total, default, allowed, fallbacks, direct_csv, standardized_csv, oc_csv, now):
    ...
    for m in models:
        ...
        direct_detail_stats = [
            ...
            stat('Host memory peak', (f"{human_size(ds.get('max_host_memory_used_bytes'))} ({pct(ds.get('max_host_memory_pct'))})" if ds and ds.get('max_host_memory_used_bytes') is not None else '—')),
            stat('GPU memory peak', (human_size(ds.get('max_gpu_memory_used_bytes')) if ds and ds.get('max_gpu_memory_used_bytes') is not None else '—')),
        ...
        oc_detail_stats = [
            ...
            stat('Host memory peak', (f"{human_size(osum.get('max_host_memory_used_bytes'))} ({pct(osum.get('max_host_memory_pct'))})" if osum and osum.get('max_host_memory_used_bytes') is not None else '—')),
            stat('GPU memory peak', (human_size(osum.get('max_gpu_memory_used_bytes')) if osum and osum.get('max_gpu_memory_used_bytes') is not None else '—')),
