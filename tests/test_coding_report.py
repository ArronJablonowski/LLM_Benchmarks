...
class CodingReportTests(unittest.TestCase):
    def test_report_accepts_only_coding_rows_and_keeps_runner_harness_matrix(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            fields = ["benchmark_suite", "model", "model_runner", "model_runner_version", "harness", "harness_version", "task_id", "verdict", "checks_passed", "checks_total", "wall_seconds"]
            with source.open("w", newline="", encoding="utf-8") as stream:
                writer = csv.DictWriter(stream, fieldnames=fields); writer.writeheader()
                writer.writerow({"benchmark_suite": "coding", "model": "model-a", "model_runner": "ollama", "model_runner_version": "0.12.0", "harness": "pi-coding-agent", "harness_version": "0.50.0", "task_id": "one", "verdict": "pass", "checks_passed": "5", "checks_total": "5", "wall_seconds": "10"})
